...
#include "runtime.h"
FILE *lm_float_dump;
static int light_leaves[MAX_MAP_LEAFS], light_leaf_count;
static directlight_t **ordered_lights;
extern int numdlights;

void IndexDirectLights(void)
{
    int leaf;
    unsigned count = 0;
    directlight_t *light;
    light_leaf_count = 0;
    free(ordered_lights);
    ordered_lights = calloc(numdlights + 1, sizeof(*ordered_lights));
    if (!ordered_lights) Error("Cannot index direct lights");
    for (leaf = 1; leaf < numleafs; ++leaf) {
        if (directlights[leaf]) light_leaves[light_leaf_count++] = leaf;
        for (light = directlights[leaf]; light; light = light->next) {
            light->lm_index = count;
            ordered_lights[count++] = light;
        }
    }
}
...
===============
*/
void LerpTriangle (triangulation_t *trian, triangle_t *t, vec3_t point, vec3_t result, LMIndirectSample *plan)
{
	patch_t		*p1, *p2, *p3;
	vec3_t		base, d1, d2;
	...
	if (plan) {
		plan->kind = 2;
		plan->patch[0] = p1 - patches;
		plan->patch[1] = p2 - patches;
		plan->patch[2] = p3 - patches;
		plan->x = x; plan->y = y; plan->x2 = x2; plan->y1 = y1;
		return;
	}
...
===============
*/
void SampleTriangulation (vec3_t point, triangulation_t *trian, triangle_t **last_tri, vec3_t result, LMIndirectSample *plan)
{
	triangle_t	*t;
	triedge_t	*e;
	vec_t		d, best;
	...
	{
		if (plan) { plan->kind = 1; plan->patch[0] = trian->points[0] - patches; }
	...
	if (*last_tri)
	{
		...
		{
			LerpTriangle (trian, *last_tri, point, result, plan);
			return;
		}
	}
	...
	for (t = trian->tris, j=0 ; j < trian->numtris ; t++, j++)
	{
		...
		// this is it
		*last_tri = t;
		LerpTriangle (trian, t, point, result, plan);
		return;
	}
	...
	for (e=trian->edges, j=0 ; j< trian->numedges ; e++, j++)
	{
		...
		if (d > 1)
			continue;
		if (plan) {
			plan->kind = 3; plan->patch[0] = p0 - patches; plan->patch[1] = p1 - patches;
			plan->x = d;
			return;
		}
	...
	if (plan) { plan->kind = 1; plan->patch[0] = p1 - patches; }
	VectorCopy( p1->totallight, result );
}

// Exact x87 fallback for the geometry-only recipes consumed by CUDA.
void lm_evaluate_indirect(const LMIndirectSample *plan, float *result)
{
	vec3_t base, d1, d2;
	int c;
	if (!plan->kind) { VectorFill(result, 0); return; }
	VectorCopy(patches[plan->patch[0]].totallight, base);
	VectorCopy(base, result);
	if (plan->kind == 1) return;
	if (plan->kind == 3) {
		for (c=0; c<3; ++c)
			result[c] = base[c] + plan->x * (patches[plan->patch[1]].totallight[c] - base[c]);
		return;
	}
	VectorSubtract(patches[plan->patch[1]].totallight, base, d1);
	VectorSubtract(patches[plan->patch[2]].totallight, base, d2);
	if (fabs(plan->x2) >= ON_EPSILON)
		for (c=0; c<3; ++c) result[c] += plan->x*d2[c]/plan->x2;
	if (fabs(plan->y1) >= ON_EPSILON)
		for (c=0; c<3; ++c) result[c] += plan->y*d1[c]/plan->y1;
}
...
void CalcFaceExtents (lightinfo_t *l)
{
	...
	for (i=0; i<2; ++i)
		if (l->texsize[i] < 0 || l->texsize[i] > 17)
...
void GatherSampleLight (vec3_t pos, byte *pvs, vec3_t normal, vec3_t *sample, byte *styles, int face, int point)
{
	int				i;
	directlight_t	*l;
	vec3_t			add;
	...
	int leaf_index, recording = lm_direct_recording(), collecting = lm_direct_prepass();
	const lm_direct_hit *hits;
	uint32_t hit_count, hit;
	lm_collect_direct(face, point, pos, normal);
	if (lm_direct_lookup(face, point, &hits, &hit_count)) {
		for (hit = 0; hit < hit_count; ++hit) {
			l = ordered_lights[hits[hit].light & LM_LIGHT_INDEX];
			if (hits[hit].light & ~LM_LIGHT_INDEX) {
				const float *sky = lm_direct_sky(face, hits[hit].sky);
				VectorCopy(sky, add);
			} else { VectorScale(l->intensity, hits[hit].ratio, add); }
			if (VectorMaximum(add) <= ((hits[hit].light & LM_SKY_DIFFUSE) ? 0 : (l->style ? coring : 0))) continue;
			for (style_index = 0; style_index < MAXLIGHTMAPS; ++style_index)
				if (styles[style_index] == l->style || styles[style_index] == 255) break;
			if (style_index == MAXLIGHTMAPS) continue;
			if (styles[style_index] == 255) styles[style_index] = l->style;
			VectorAdd(sample[style_index], add, sample[style_index]);
		}
		return;
	}

	for (leaf_index = 0; leaf_index < (lm_reference_mode ? numleafs-1 : light_leaf_count); ++leaf_index)
	{
		i = lm_reference_mode ? leaf_index + 1 : light_leaves[leaf_index];
		if ( (l = directlights[i]) && (pvs[ (i-1)>>3] & (1<<((i-1)&7))) )
		{
			for (; l ; l=l->next)
			{
				int lm_visible;
				if (collecting && l->type == emit_skylight) continue;
				lm_visible = l->type == emit_skylight ? -1 : lm_direct_visibility(face, point, l->lm_index, pos, normal);
				if (lm_visible == 0 && !lm_audit_gpu_direct) continue;
				...
				if (collecting) {
					lm_add_direct_ray(face, l->lm_index, pos, l->origin);
					continue;
				}
				if (lm_audit_gpu_direct && l->type != emit_skylight && lm_visible >= 0 && lm_visible != 2) {
					int cpu_visible = TestLine_r(0, pos, l->origin) == CONTENTS_EMPTY;
					if ((lm_visible == 1) != cpu_visible)
						Error("CUDA direct mismatch face %d point %d light %u: (%a %a %a) -> (%a %a %a), result %d type %d dots %a %a normals (%a %a %a) (%a %a %a) stop %a", face, point, l->lm_index, pos[0], pos[1], pos[2], l->origin[0], l->origin[1], l->origin[2], lm_visible, l->type, dot, l->type == emit_point ? 0 : dot2, normal[0], normal[1], normal[2], l->normal[0], l->normal[1], l->normal[2], l->stopdot2);
				}
				if (recording) {
					if (l->type == emit_skylight) lm_direct_record_sky(face, l->lm_index, add, 0);
					else {
						if (lm_visible != 1 && TestLine_r(0, pos, l->origin) != CONTENTS_EMPTY) continue;
						lm_direct_record(face, l->lm_index, ratio);
					}
				}
				if( VectorMaximum( add ) > ( l->style ? coring : 0 ) )
				{
					if ( !recording && lm_visible != 1 && l->type != emit_skylight && TestLine_r (0, pos, l->origin) != CONTENTS_EMPTY )
						continue;	// occluded
	...
	if (sky_used && indirect_sun != 0.0)
	{
		...
		{
			if (recording) lm_direct_record_sky(face, sky_used->lm_index, total, 1);
			for( style_index = 0; style_index < MAXLIGHTMAPS; style_index++ )
...
=============
*/
void AddSampleToPatch (sample_t *s, int facenum, int included)
{
	patch_t	*patch;
	vec3_t	mins, maxs;
	int		i;
	...
	for (patch = face_patches[facenum] ; patch ; patch=patch->next)
	{
		...
		if (lm_reference_mode) WindingBounds (patch->winding, mins, maxs);
		else { VectorCopy(patch->mins, mins); VectorCopy(patch->maxs, maxs); }
		for (i=0 ; i<3 ; i++)
		{
...
void BuildFacelights (int facenum)
{
	...
	int lm_point = 0;
	int lm_collecting = lm_direct_prepass();
...
//
	...
	for (k=0 ; !lm_collecting && k<MAXLIGHTMAPS; k++)
	...
	for (i=0 ; i<l.numsurfpt ; i++, spot += 3)
	{
			...
			for ( t = -1; t <= 1; t ++ )
			{
				for ( s = -1; s <= 1; s++ )
				{
					  ...
					  && (0 <= t+sample_t) && (t+sample_t < lightmapheight) )
					{
						...
						GatherSampleLight( pos, pvs, pointnormal, subsampled, f->styles, facenum, lm_point++ );
		...
		else
		{
			...
			GatherSampleLight( spot, pvs, pointnormal, sampled, f->styles, facenum, lm_point++ );
		}

		if (lm_collecting) continue;
		for( j=0; j < MAXLIGHTMAPS && (f->styles[j] != 255); j++ )
		{
			VectorCopy (sampled[j], facelight[facenum].samples[j][i].light );
			if ( f->styles[j] == 0 )
			{
				int included = lm_sample_membership(facenum, i, VectorAvg(facelight[facenum].samples[j][i].light) >= 1);
				AddSampleToPatch ( &facelight[facenum].samples[j][i], facenum, included);
			}
		}
	}

	if (lm_collecting) return;
...
=============
*/
static triangulation_t *BuildFaceTriangulation(int facenum)
{
    dface_t *f = &dfaces[facenum], *f2;
    patch_t *patch;
...
{
    if (texinfo[dfaces[face].texinfo].flags & TEX_SPECIAL) return 0;
    return facelight[face].numsamples;
}

const float *lm_face_sample_position(int face, int sample)
{
    return facelight[face].samples[0][sample].pos;
}
...
void lm_prepare_indirect_plan(LMIndirectSample *plans, const uint32_t *offsets)
{
    ...
    for (face=0; face<numfaces; ++face) {
        triangulation_t *trian;
        triangle_t *last = NULL;
        vec3_t ignored;
        if (offsets[face] == offsets[face+1]) continue;
        trian = BuildFaceTriangulation(face);
        for (sample=0; sample<facelight[face].numsamples; ++sample)
            SampleTriangulation(facelight[face].samples[0][sample].pos, trian, &last, ignored,
                                &plans[offsets[face]+sample]);
...
void FinalLightFace (int facenum)
{
	...
	float		minlight;
	int			lightstyles;
	int audit_indirect = getenv("LM_AUDIT_GPU_INDIRECT") != NULL;
	...
	trian = NULL;
	if (numbounce > 0 && (!lm_indirect_ready() || audit_indirect))
		trian = BuildFaceTriangulation(facenum);
	//
	...
	{
		if(lm_float_dump) { int header[3]={facenum,f->styles[k],fl->numsamples}; fwrite(header,sizeof(int),3,lm_float_dump); }
		last_tri = NULL;
		samp = fl->samples[k];
		for (j=0 ; j<fl->numsamples ; j++, samp++)
		{
			...
			if(lm_float_dump) fwrite(samp->light,sizeof(float),3,lm_float_dump);
			VectorFill(v,0);
			VectorScale( samp->light, 2.0, lb ); 

			if (numbounce > 0 && k == 0 )
			{
				if (!lm_indirect_sample(facenum, j, v))
					SampleTriangulation (samp->pos, trian, &last_tri, v, NULL);
				else if (audit_indirect) {
					vec3_t expected;
					SampleTriangulation(samp->pos, trian, &last_tri, expected, NULL);
					if (memcmp(v, expected, sizeof expected))
						Error("CUDA interpolation plan differs from QRAD at face %d sample %d", facenum, j);
				}
				VectorAdd( lb, v, lb );
			}

			if(lm_float_dump) fwrite(v,sizeof(float),3,lm_float_dump);
			...
			if (light_gamma != 1.0)
				for( i=0; i<3; i++ )
					lb[i] = (float)pow( lb[i] / 256.0f, light_gamma ) * 256.0f;
