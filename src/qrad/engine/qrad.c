...
{
    return ((emission[0] != 0 || emission[1] != 0 || emission[2] != 0) ? 1 : 0) |
        (VectorAvg(emission) >= dlight_threshold ? 2 : 0);
}
...
void BounceLight (void)
{
	...
	{
		if (!lm_gpu_gather()) RunThreadsOn (num_patches, true, GatherLight);
		CollectLight( added );
...
readtransfers(char *transferfile, long numpatches)
{
	...
	if ( (handle = _open( transferfile, _O_RDONLY | _O_BINARY )) != -1 )
	{
			...
			if ( filepatches == numpatches )
			{
				...
				for( patch = patches; readpatches < numpatches; patch++ )
				{
					  ...
					  == sizeof(patch->numtransfers) )
					{
						...
						{
							printf("\nMemory allocation failure creating transfer lists(%d*%zu)!\n",
								  patch->numtransfers, sizeof(transfer_t) );
							break;
						}
					}
		...
		_close( handle );
		time(&end);
		printf("%10.3fMB] (%.0f)\n",totalbytes/(1024.0*1024.0), difftime(end, start));
	}
...
void RadWorld (void)
{
	...
	if (numbounce > 0)
	{
		lm_mark(LM_TRANSFERS);
		// build transfer lists
		if (!lm_transfers_load()) {
			MakeAllScales ();
			// Cache incoming transfers after preserving the original swap order.
...
int qrad_main (int argc, char **argv)
{
	...
	for (i=1 ; i<argc ; i++)
	{
		...
		else if (!strcmp(argv[i],"-gamma"))
		{
			if ( ++i < argc )
			{
				light_gamma = (float)atof (argv[i]);
			}
			else
			{
				fprintf( stderr, "Error: expected a value after '-light_gamma'\n" );
				return 1;
			}
		}
	...
	if (i != argc - 1)
		Error ("usage: qrad [-dump] [-inc] [-bounce n] [-threads n] [-verbose] [-terse] [-chop n] [-maxchop n] [-scale n] [-ambient red green blue] [-proj file] [-maxlight n] [-threads n] [-lights file] [-light_gamma n] [-dlight n] [-extra] [-smooth n] [-coring n] [-notexscale] bspfile");
	...
	/* Reconstruction build: no implicit RAD discovery. Only explicit -lights. */
	if (*designer_lights) ReadLightFile(designer_lights);
	...
	lm_mark(LM_LOAD);
	LoadBSPFile (getenv("LM_BSP_INPUT") ? getenv("LM_BSP_INPUT") : source);
	ParseEntities ();
	lm_initialize();
	...
	{
		extern FILE *lm_float_dump;
		const char *fd = getenv("LM_FLOAT_FD");
		if (fd) {
			lm_float_dump = fdopen(atoi(fd), "wb");
			if (!lm_float_dump) Error("Cannot open in-memory lighting output");
		}
		RadWorld();
		if (lm_float_dump && fclose(lm_float_dump)) Error("Cannot finish in-memory lighting output");
	}
	...
	lm_mark(LM_WRITE);
	if (!getenv("LM_NO_BSP_WRITE")) WriteBSPFile (source);
