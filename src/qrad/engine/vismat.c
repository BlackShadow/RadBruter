	...
	if ( patch2 && DotProduct(patch->origin, patch2->normal) > PatchPlaneDist(patch2)+1.01 )
	{
		...
		for ( ; patch2 ; patch2 = patch2->next)
		{
			  ...
			  && DotProduct (patch2->origin, patch->normal) > PatchPlaneDist(patch)+1.01 )
			{
				...
				int bitset = bitpos+m;
				if (lm_enqueue_patch_ray(head, patch->origin, patch2->origin, bitset)) continue;
				if (TestLine_r(head, patch->origin, patch2->origin) != CONTENTS_EMPTY) continue;
...
getfiledata(char *filename, char *buffer, int buffersize)
{
	...
	if ( (handle = _open( filename, _O_RDONLY | _O_BINARY )) != -1 )
	{
		...
		_close( handle );
		time(&end);
		printf("%10.3fMB] (%.0f)\n",size/(1024.0*1024.0), difftime(end, start));
	}
...
_int64 getfreespace(char *filename) {
#ifdef _WIN32
    ULARGE_INTEGER available;
    if (!GetDiskFreeSpaceExA(".", &available, NULL, NULL)) return 0;
    return (_int64)available.QuadPart;
#else
    struct statvfs v;
    if (statvfs(".", &v)) return 0;
    return (_int64)v.f_bavail * v.f_frsize;
#endif
}
